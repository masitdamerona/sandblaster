...
def get_filter_arg_syscall_number(f, arg):
    ...
    with open('syscalls.json') as data:
        syscalls = json.load(data)

        for syscall in syscalls.get("bsd_syscalls", []):
            if syscall.get("number") == arg:
                return '%s' % (syscall.get("name"))
...
def get_filter_arg_syscall_number_bitmask(f, offset):
    ...
    syscalls_f = open('syscalls.json', 'rb')
    syscalls = json.load(syscalls_f)
    ...
    enabled_syscalls = []
    for byte_index, byte in enumerate(bitmap):
        for bit_position in range(0, 8, 1):
            mask = (byte >> bit_position) & 0b1
            if mask:
                syscall_number = (byte_index * 8) + bit_position
                for syscall in syscalls.get("bsd_syscalls", []):
                    if syscall.get("number") == syscall_number:
                        syscall_name = syscall.get("name", f"UNKNOWN_syscall_number_{syscall_number}")
                        enabled_syscalls.append(syscall_name)
